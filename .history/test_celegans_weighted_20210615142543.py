...
def update_state():
    ...
    if nbsize != 0:
        ...
        print(g.get_out_neighbors(g.get_in_neighbors(spiker)))
        secnd_nbhood = g.get_out_neighbors(g.get_in_neighbors(spiker))
        print(secnd_nbhood)
        new_nb = npr.choice(secnd_nbhood)
        if new_nb != spiker:
            e = g.add_edge(spiker, new_nb)
            g.ep.weight[e] = g.vp.activation[spiker]/2
        g.vp.activation[spiker] *= 1/2            
...
# %%
nbs = g.get_out_neighbors(g.get_in_neighbors(1))
